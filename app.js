...
app.use(expressJWT({
    ...
    path: ['/college/login', '/admin/login','/verifier/getfile']
}));
app.use(bearerToken());

app.use((req, res, next) => {
    console.log('New req for %s', req.originalUrl);
    if (req.originalUrl.indexOf('/verifier/getfile') >= 0 ||req.originalUrl.indexOf('/admin/login') >= 0 || req.originalUrl.indexOf('/college/login') >= 0) {
        return next();
    }
    var token = req.token;
